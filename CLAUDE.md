...
**当前线上 = v12.14（2026-07-20）。实时化已升为正式主版本。多语言：✅已缩到6语(EN/DE/FR/JA/zh-CN/zh-TW，两处网格删KO/ES/PT/TR+老档迁移)、✅歌曲金句6语、✅首页日期行原生6语。⏳ 全量本地化仍是长尾（index.html ~549处 isZh 三元 + ~248处 lang==='zh' + 18个NPC_BIO(多zh/en) + story.js ~64处 zh-cn键缺de/fr/ja）——但 de/fr/ja 玩家现在=原生AI对话(prompt强制目标语)+UI回退英文，功能可用不破。原生UI打磨是低优先长尾，可增量做(设置标题→NPC资料卡→场景标签…)。`main` 分支根目录 = v11.98 实时化 = 线上正式版（`…/off-the-record/`）；`beta/` 子目录 = 同版 v11.98 沙盒试验田（开发先在这改、验证 OK 再同步/升级到根目录）。** 二者同域名共用 localStorage，故**根目录正式版用干净 key（`otr_save_v2`/`otr_device_id`），beta/ 沙盒用 `_beta` 后缀 key**，存档互不污染——改动时勿混。旧的 v10.95 有限剧情版已退役（被根目录覆盖，git 历史里可查）。废弃的 `realtime` 分支（早期 v10.97 实时化尝试）已删除。
