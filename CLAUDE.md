...
**当前版本（2026-07-19 起）：实时化已升为正式主版本。`main` 分支根目录 = v11.98 实时化 = 线上正式版（`…/off-the-record/`）；`beta/` 子目录 = 同版 v11.98 沙盒试验田（开发先在这改、验证 OK 再同步/升级到根目录）。** 二者同域名共用 localStorage，故**根目录正式版用干净 key（`otr_save_v2`/`otr_device_id`），beta/ 沙盒用 `_beta` 后缀 key**，存档互不污染——改动时勿混。旧的 v10.95 有限剧情版已退役（被根目录覆盖，git 历史里可查）。废弃的 `realtime` 分支（早期 v10.97 实时化尝试）已删除。
