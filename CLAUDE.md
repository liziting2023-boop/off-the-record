...
**⚡ 30秒速览**：正处**20人×2周封测筹备**尾声。封测基础设施**已全部就绪**（测试码门+同步码+双反馈表单+进度面板 `/admin`+邀请二维码+report端点）。**开测前只差两个内容大件**（用户已拍板、待开发）：**① 特级NPC·足球明星 Rafael 样板**（先做，见 `npc_special.md`）**② 群聊**（乐队群/团队群，NPC按 `NPC_ACQUAINTANCE` 关系网自动接话；不做"追求者擂台"）。**另一独立大件**：多语言总攻（缩到6语EN/DE/FR/JA/zhCN/zhTW+全量本地化，用户会另开窗口时说做）。工作流铁律：改代码 → 三处版本号+script `?v=` 同步升 → `node` 语法检查 → commit&push main（GitHub Pages 从 main 发布，push 即上线）。**根目录=玩家正式版**，`beta/`=沙盒；重大改动可先在 beta/ 试、再同步到根目录（复制 beta/三文件→根目录，并把 `_beta` key 换成干净 key：`sed 's/otr_save_v2_beta/otr_save_v2/g; s/otr_device_id_beta/otr_device_id/g'`）。日常小改也可直接改根目录（它就是产品）。**改 worker.js 后**：`export $(grep -v '^#' /d/OTR/cloudflare_wrangler.env|xargs) && npx wrangler deploy --config /d/OTR/wrangler.toml`（凭据/配置在仓库外，勿删 keep_vars）。生图铁律：提示词里**任何位置**出现 "hands" 都是怪手吸引子，构图只走 `PORTRAIT_CROP`。

**当前线上 = v12.16（2026-07-20）。实时化已升为正式主版本。**
**多语言总攻进度（用户选B=增量啃原生UI，一块块推）**：✅缩到6语(EN/DE/FR/JA/zh-CN/zh-TW，onboarding+设置两处网格删KO/ES/PT/TR，applyLang迁移老档)、✅歌曲金句6语(STORY.songs hook_de/fr/ja + state.js musicNote按lang挑)、✅首页日期行6语(days_de/fr+Tag/Jour)、✅设置区块标题+同步码/反馈区6语(L()已扩到6参:zh,en,ja,ko,de,fr)、✅名气等级FAME_DATA+底部导航+压力标签6语。**途中修的真bug**：getFameLabel缺翻译时回退中文(改回退英文)；设置标题/同步码/反馈原来硬编码英文(中文玩家也看英文)。**⏳ 剩余长尾(按可见度继续)**：①NPC_BIO 18个资料卡(多zh/en,render在openNpcProfile按isZh挑,要加de/fr/ja+改render) ②场景/日历/灵感提示等散串(~549处isZh三元,de/fr当前回退英文=可读不破) ③story.js ~64处zh-cn键缺de/fr/ja(preText/label,已有 `[lang]||['en']` 回退)。**基础设施**：`L(zh,en,ja,ko,de,fr)`已6参(applyLang内)；`t()`走TX表(TX.de/fr稀疏,回退en)；`_L`(3723附近)仍4参未扩。**下一块建议**：NPC资料卡。**⚠️ 用户已积压超大批未真机验收**(第3波11组+助理NPC+仪式感模板+多语言5块)，建议提醒先测一轮。`main` 分支根目录 = v11.98 实时化 = 线上正式版（`…/off-the-record/`）；`beta/` 子目录 = 同版 v11.98 沙盒试验田（开发先在这改、验证 OK 再同步/升级到根目录）。** 二者同域名共用 localStorage，故**根目录正式版用干净 key（`otr_save_v2`/`otr_device_id`），beta/ 沙盒用 `_beta` 后缀 key**，存档互不污染——改动时勿混。旧的 v10.95 有限剧情版已退役（被根目录覆盖，git 历史里可查）。废弃的 `realtime` 分支（早期 v10.97 实时化尝试）已删除。
...
**v12.06–v12.11（2026-07-20 第3波反馈，11组全部上线）**：①**女主人设重塑**：不再又穷又惨，改"为梦想/被听见而唱、经济无忧的艺术家"（flashback + STORY.characters.protagonist）。②**歌曲圣经** `STORY.songs`（Tonight's Light/Off the Record/Louder/Keep the Light On，英文正典+中文金句，德法日进多语言总攻）注入 agent/drummer/rival 对话提示，防乱编歌词。③**签约剧本大修**：删推咖啡/重复递名片/"语气平静"冷描述、时间线统一、"3-4天"→"most days"、加"工作可改期/有些锁死"、引用真歌词。④**开局消息时机**：Day1消息必须等**经纪人+鼓手都见完**(drummer.met)+1分钟才发（修见面时穿帮发短信）。⑤**族裔**：经纪人开局选择器加"✦混血"(selCard `_agentMixMode`)；`AGENT_ORIGIN_VALS`+npcAppearance 加 Greek/Portuguese/Dutch/Irish（4语数组必须等长）。⑥**Rafael** `NO_ORIGIN_PICK` 跳过选族裔发色、直接出3图选1（`_showSecondaryIntro` 分支）。⑦**公寓**：改宽敞现代(LIVING_LAYOUT×3去掉budget/thrifted)，`DECOR_PROMPTS` 把所选装修风格真正注入房间生图(修"不同风格出图一样")。⑧**生图**：女主服装按场景+天数确定性换装(buildPlayerPrompt `_outfits`)；**`IMG_DEV_PORTRAITS=true`**(人物图走flux/dev,PORTRAIT_CROP自动失效,背景仍schnell)。⑨**看画面**：去掉按钮、点背景图toggle(vn-bg onclick已有)、hide规则加 #portrait-full/#npc-trio。⑩**对话**：台词`.speech`改红#FF5C78加粗；灵感语气按钮改3列网格。⑪**重要节点仪式感** `playMilestone(type)`：全屏章节卡+主题渐变+`playMilestoneChord`合成音效(WebAudio零文件)，7类intimate/confess/cohabit/propose/marry/company/award；已接表白/第一次过夜/同居三个现有触发点，其余模板待接。DEV面板加"🎬节点"预览7类+"恋人化/初见"测试按钮。**⚠️未完成的2个"觉得合适就改"小项**：(a)玩家输入的"动作白斜体/说话红字/去括号"呈现约定只做了NPC台词标红，玩家自己输入的解析未做；(b)签约旁白→办公室场景的过渡(#14)未加(签约不在7类里)。
**v11.99–v12.05（2026-07-19 大批量玩家反馈，11组全部上线）**：①**日历Bug**：`weekdayOffsetFromText` 加"明天/后天/tomorrow"相对词解析（治约会排错天）；约会解锁窗口30分→**提前1小时**（`inviteState`）；日历条目去台词、改显示地点（`locDisplayName`）。②**UI**：约会输入框放大（`.vn-input`）；回忆相册3列→**2列**；首页"还剩0次/还有3次"矛盾修（0次时不显示数字）+顶部浅色罩治看不清；③**开局**：开场stamp去掉星期（`open_stamp`）；`day1Intro` 改序章一结束经纪人+鼓手各发1条、管家+10min，用真实时钟，回主页即触发（不再等夜里）。④**管家 Finn 改暑假打工门房/concierge**（原实习楼管）——`NPC_ROLES/NPC_BIO/EARLY_TONE/day1Intro/T2_INTRO/state.js secondaryLooks-无(用buildButlerPrompt)` 全改；story.js profession/culture/arc 同步；加"做客礼数"治反客为主。⑤**聊天**：`buildDialoguePrompt` 禁"吃了吗"开场+话题按阶段演进（全NPC全语言）；鼓手加演唱技巧等话题；资料卡加**来自哪里**（`originDisplayName`）；灵感提示扩池。⑥**场景兑现**：`playInviteScene` 兑现邀约许诺的话题（work邀约不再被"纯约会不谈工作"盖掉）+生日点题；`runMeetupScene` 加生日+接送方向（他送她、演到目的地）；加 airport/car_ride/street_walk 动作提示；`isTodayPlayerBirthday()`。⑦**工作系统**：密度3→5天/周+时段随机（早/午/晚）；**经纪人聊天里可真的改/取消工作**（结构化 `reschedule` 字段→改日历，修"口头答应日历没变"）；合同拍摄`locked`不可改；录音行程+鼓手好感够→自动加当晚"一起吃饭"（`maybeAddPostWorkDinner`）。⑧**表白后三件套**：里程碑卡（`showConfessMilestone`）+正式恋人称呼变亲昵/更主动（`confessedNote`+主动+0.2）+高好感**同居邀请**（`maybeCohabInvite`→`_cohabiting`→`cohabitingNote`）。⑨**PWA图标**：`icon.svg`+`manifest.webmanifest`+head链接（治加主页显示"G"）。⑩**混血可选项**：族裔选择加"✦混血"模式选两个来源→存'A×B'，卡片本地化显示，生图`imgOriginText/App`转"mixed A and B"。⑪**两个新一级NPC**：**球星 Rafael**（贝林厄姆启发的独立虚构角色，Day6饮料广告拍摄`_introNpc`路由结识，`STAYOVER_MIN`34）+**邻居单亲爸爸**（35岁带6岁女儿Lily，Day3走廊必遇，`STAYOVER_MIN`42）——都走 secondaryLooks 生图（dispatch门槛放宽为`!prompt&&secondaryLooks[k]`）、`_showSecondaryIntro`初见、`_deepMerge`自动补进老存档。**待用户真机验收**：两NPC初见流程（选族裔→三选一→场景）、经纪人改期、同居邀请触发、生图是否够帅。
...
**v11.98（2026-07-19）**：①**日历丢失双重加固**（用户复现"重进游戏工作全更新、约会全没"，根因仍未定位——复发让用户跑 otrDiag）：读档时从聊天消息的 meet.invId **重建全部已接受约会**（权威记录，id去重）；排班改 `schedRand`（`_schedSeed`×天数确定性随机）——**丢档重排=一模一样的日程**，"工作全变了"症状不可能再现。②好感进度条加**亲密线刻度**（STAYOVER_MIN 处竖线，过线填充变深红#E0356B）。③设置页语言/画风改**点"保存并返回"才生效**（pending 暂存，openSettings 清）。④主动消息**禁"吃了吗/还醒着/累不累"开场** + GETTOKNOW_SEEDS 扩6条（工作趣事/二选一/想到你/小决定/打趣/今日所见）。⑤删重复 Photorealistic 卡、删独立 Copy debug 按钮（表单按钮自动复制+失败弹窗兜底）、privacy.html 改 17+。**📌 待办·多语言总攻（用户已定稿 2026-07-19，等用户开新窗口时做）**：语言从10种缩到 **6 种：EN / DE / FR / JA / zh-CN / zh-TW**（删 KO/ES/PT/TR，两处语言网格都要删）；对这6种做**全量本地化**——已知漏网点：首页日期行+“新人艺人/压力”标签（现只有zh/en）、设置页区块标题（LANGUAGE/IMAGE STYLE/SYNC CODE/BETA FEEDBACK 等硬编码英文）、NPC信息卡 Met/简介（NPC_BIO 只有zh/en）、日历"这天没有安排"等散串。方法：全库扫 `isZh?` 三元和硬编码中文串，统一走 t() 翻译表补全6语。注：诊断实测用户档 lang=es 即误触旧即时切换所致（v11.98已改保存生效）。①**桌面端锁手机尺寸**：≥600px 时 body 收窄 430px 居中+暗色剧场背景；`.screen` 流内自然被包住，所有 `position:fixed` 浮层（底栏/各全屏 overlay/年龄门/测试码门/灯箱/dev面板）用 `!important` 拉回框内（盖得过内联样式），四角固定按钮用 calc() 偏移。②设置页底部整理成两张卡（SYNC CODE / BETA FEEDBACK）纯英文标签+一行页脚（隐私·邮箱）。③`BUG_FORM_URL` 已填（https://forms.gle/W6p4bxHW4vxZfKmU8，带截图上传→强制Google登录属预期），🐛按钮生效。④邀请二维码已生成：**D:\OTR\beta_invite_qr.png**（指向 /beta/）。付费第4题用户决定不补。
...
**v11.88（2026-07-14）**：①**表白反馈**：表白只在已亲密后解锁→无"失败"结局，他的台词=按人设的yes；AI note 加"答案必须 UNMISTAKABLE、不许让她猜"；系统落点=首次表白 +6 好感、rel-up 音效、写入记忆（"心意已挑明"，此后对话以此为前提）。②好感度页 **💗 移除**（用户拍板：表白按钮亮起即提示，图标重复）；🌙 保留。③ADDRESS RULE 加固：禁编"艺人档案/机构记录"当知道地址的借口（教练实测编了这个）。④**二线暗姓**：`SECONDARY_SURNAMES` 池，命名/读档自愈时定死一个姓；UI 仍只显名，但全名注入 buildDialoguePrompt——被问姓永远答同一个（不再现编现忘）；三线不配姓。

**v11.87（2026-07-14）地址常识**：健身教练好感25（≥门槛20→homeVisitAllowed放行）回"你在哪儿我去接你"、散步场景直接开在她家——**到亲密门槛≠知道她住哪**（她从没给过地址）。①聊天 prompt 加 **ADDRESS RULE**：只有管家（她楼里工作）/经纪人（签约资料）/已过夜的知道她家，其他人必须"到场地碰面"，除非她本次聊天主动给地址/邀请。②代码兜底：聊天 meet 与 futureInvite 两条路里，`her_home` 且非管家/经纪人/nights=0 → 改约 park。
...
**v11.83（2026-07-13）离线补发·真实发送时间**（用户定稿：NPC想什么时候发就什么时候发，上线看到的是攒下的消息，时间戳=他实际发送时刻，不再全标上线那一刻）：`checkProactiveOnOpen` 重构——记录 `G._lastOnlineTs`，离线≥90分钟→最多3个过门槛的NPC"在你不在时发过"，各随机落在离线窗口内（只回溯今天、最早7:00）且该NPC**当时醒着**（新 `isNpcAsleepAt(k,min)`，`isNpcAsleep` 改为其包装）；提示词按发送时刻定时段措辞/时钟+注入"她现在不在手机旁，别期待秒回"；`_reachAt` 冷却按发送时刻算；消息直接以 `fmtClock(sendMin)` 落时间戳（不走 nextMsgTime 排队）。在线短间隔仍旧单挑一人、实时时间。生成逻辑抽成 `_sendProactive(k,sendMin,now,nowMin,backfill)`。
...
| 保镖(detective键) | 36 | 名气≥30/第7天 | 高挑 | 退役特种兵贴身安保；有个小儿子由妹妹带大 |

### 隐藏剧情
- 管家是经纪人**同父异母**弟弟（私生子背景）——**保留**，但已改为【良性设定】：两人没一起长大、不亲近、只是客气的远亲，**没有秘密、没有阴谋**，被问就大方坦白（story.js butler hiddenTruths 701/707）。
- ⚠️ **已废弃（2026-07-24 用户拍板不要了）**：原"管家雇侦探调查经纪人找把柄要挟 + 第210天出生证明揭秘"这条隐藏调查/揭秘线**整条删除**。章节大纲(chapters)里相关 intrigue 事件（侦探警告/侦探经纪人打架/墓碑照片/剪报/出生证明揭秘）已全部改成中性的恋爱+事业节拍。
- **侦探已改为"保镖"**（detective 键复用）：退役特种兵贴身安保，名气≥30 或第7天由经纪公司配发登场（非原 Day40 出租车神秘现身）。
